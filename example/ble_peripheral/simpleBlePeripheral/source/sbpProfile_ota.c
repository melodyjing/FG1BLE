	...
	if ( pAttr->type.len == ATT_BT_UUID_SIZE )
	{
		...
		switch ( uuid )
		{
			...
			case SIMPLEPROFILE_CHAR2_UUID:
				*pLen = ReadNotify_Len;//BLE_ATT_CMD_LED;
                                 ...
                                 uint8 *pValue, uint16 len, uint16 offset )
{
	...
	// If attribute permissions require authorization to write, return error如果属性权限需要授权才能写入，则返回错误
	...
	if ( pAttr->type.len == ATT_BT_UUID_SIZE )
	{
		...
		switch ( uuid )
		{
			case SIMPLEPROFILE_CHAR1_UUID:
				...
				//Write the value
				if ( status == SUCCESS )							//连接成功
				{

					for(u8 i=0 ; i<len; i++)
					{
						device_all_data.dev_app_push_data[i] = pValue[i] ;
					}
					device_all_data.dev_app_push_data[39] = len ;
					device_all_data.dev_app_push_data[38] = 2 ;
					osal_set_event(APP_TaskID, UI_100MS_EVENT_TICK);
...
bStatus_t simpleProfile_Notify( uint8 param, uint8 len, void *value )		//透传服务通知
{
	bStatus_t ret = SUCCESS;
	uint16 notfEnable;
