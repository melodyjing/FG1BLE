...
typedef struct
{
  pfnGATTReadAttrCB_t pfnReadAttrCB;           //!< Read callback function pointer  读取回调函数指针
  pfnGATTWriteAttrCB_t pfnWriteAttrCB;         //!< Write callback function pointer 写入回调函数指针
  pfnGATTAuthorizeAttrCB_t pfnAuthorizeAttrCB; //!< Authorization callback function pointer授权回调函数指针
} gattServiceCBs_t;
