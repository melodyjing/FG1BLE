/***********************************************************************
	...
	Date:	  2022-5-31
	Author:   melody
	Description:This file contains all functions support for pwm driver. 
************************************************************************/
...
void hal_pwm_init(
    PWMN_e pwmN,					//PWM 通道
    PWM_CLK_DIV_e pwmDiv,			//PWM 通道时钟预分频
    PWM_CNT_MODE_e pwmMode,			//PWM 通道的计数模式
    PWM_POLARITY_e pwmPolarity)		//PWM 通道的输出极性设置
{
...
{
    if(cmpVal > cntTopVal)				//比较值>顶部值
        return;

    PWM_NO_LOAD_CH(pwmN);
    PWM_SET_CMP_VAL(pwmN, cmpVal);		//PWM通道的比较值
    PWM_SET_TOP_VAL(pwmN, cntTopVal);	//PWM通道的计数器顶部值
    PWM_LOAD_CH(pwmN);
}
...
void hal_pwm_start(void)
{
    if(pwm_en == 0)
    {
        hal_pwrmgr_lock(MOD_PWM);		//设置PWM模块的LOCK状态为TURE,并禁止休眠
        PWM_ENABLE_ALL;
        pwm_en = 1;
    }
}
...
void hal_pwm_stop(void)
{
    if(pwm_en == 1)
    {
        hal_pwrmgr_unlock(MOD_PWM);		//设置PWM模块的LOCK为False,并使能修眠
        PWM_DISABLE_ALL;
        pwm_en = 0;
        hal_clk_gate_disable(MOD_PWM);	//关闭PWM模块时钟
    }
}
