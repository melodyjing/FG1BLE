...
// Minimum connection interval (units of 1.25ms, 80=100ms) if automatic parameter update request is enabled最小连接间隔40ms
#define DEFAULT_DESIRED_MIN_CONN_INTERVAL		32//32//80
// Maximum connection interval (units of 1.25ms, 800=1000ms) if automatic parameter update request is enabled最大连接间隔60ms
#define DEFAULT_DESIRED_MAX_CONN_INTERVAL		48//48//800
// Slave latency to use if automatic parameter update request is enabled启用自动更新参数请求时所使用的从属延迟
#define DEFAULT_DESIRED_SLAVE_LATENCY			0
// Supervision timeout value (units of 10ms, 1000=10s) if automatic parameter update request is enabled 启用自动更新时，监控超时值 3S
#define DEFAULT_DESIRED_CONN_TIMEOUT			300//1000
// Whether to enable automatic parameter update request when a connection is formed建立连接时是否启用自动更新参数请求
#define DEFAULT_ENABLE_UPDATE_REQUEST			TRUE
// Connection Pause Peripheral time value (in seconds)
#define DEFAULT_CONN_PAUSE_PERIPHERAL			6		//6S
#define INVALID_CONNHANDLE						0xFFFF
// Default passcode
#define DEFAULT_PASSCODE						0//19655	默认密码
// Length of bd addr as a string
#define B_ADDR_STR_LEN							15
#define RESOLVING_LIST_ENTRY_NUM				10
// Offset of advertData&scanRspData							//广告数据和扫描数据的偏移值
...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "LANHE-TEST";
/*********************************************************************
 * LOCAL FUNCTIONS
 */
...
//	uint8	i	=	0;
	...
	uint16 gapRole_AdvertOffTime	=	0;						//2022/7/5
...
//	LOG("events:0x%x\n",events);
    ...
    if ( events & SYS_EVENT_MSG )
    {
        ...
        if ( (pMsg = osal_msg_receive( simpleBLEPeripheral_TaskID )) != NULL )
        {
            ...
            // Release the OSAL message释放OSAL消息
            VOID osal_msg_deallocate( pMsg );
        }

        // return unprocessed events 返回未处理事件
 ...
 *
 * @brief   Notification from the profile of a state change.来自配置文件的状态更改通知
 *
 ...
 static void peripheralStateNotificationCB( gaprole_States_t newState )
{
    switch ( newState )
    {
        case GAPROLE_STARTED:
        {
            ...
            LOG("Gaprole_start:\n");
//			HCI_PPLUS_AdvEventDoneNoticeCmd(APP_TaskID, UI_BLE_TIMEOUT_TICK_2) ;
//			osal_set_event(simpleBLEPeripheral_TaskID, SBP_RESET_ADV_EVT);                                     
        ...
        case GAPROLE_CONNECTED:
            HCI_PPLUS_ConnEventDoneNoticeCmd(APP_TaskID, UI_BLE_TIMEOUT_TICK_1); 
...
void set_simpleBLE_time(u16 time)
{
	uint8 initial_advertising_enable = FALSE;
	GAPRole_SetParameter( GAPROLE_ADVERT_ENABLED, sizeof( uint8 ), &initial_advertising_enable );
	uint16 advInt = time;
