...
/*********************************************************************
    ...
    @brief   Find service's callback record.查找服务的回调记录
...
/*********************************************************************
    ...
    @brief       GATT Server App message processing function.GATT服务器APP报文处理功能
...
/*********************************************************************
    ...
    @brief       Process Execute Write Request.处理执行写入请求

    @param       pMsg - pointer to received message指向收到消息的指针
    @param       pErrHandle - attribute handle that generates an error
...
static bStatus_t gattServApp_ProcessExecuteWriteReq( gattMsgEvent_t* pMsg, uint16* pErrHandle )
{
    ...
    uint8 status = SUCCESS;
    // See if this client has a prepare write queue查看此客户端是否有准备写入队列
    ...
    if ( pQueue != NULL )
    {
        for ( uint8 i = 0; i < maxNumPrepareWrites; i++ )
        {
            ...
            // See if there're any prepared write requests in the queue查看队列是否有任何已准备好的写入请求
        ...
        // Mark this queue as empty将此队列标记为空
...
/*********************************************************************
    ...
    @brief       Enqueue Prepare Write Request.准备写入队列请求
...
{
    prepareWrites_t* pQueue;
    // First see if there's queue already assocaited with this client首先查看是否有与此客户端相关联的队列
    pQueue = gattServApp_FindPrepareWriteQ( connHandle );

    if ( pQueue == NULL )
    {
        // Find a queue for this client查找此客户端的队列
    ...
    // If a queue is found for this client then enqueue the request如果找到此客户端的队列，则将请求排入队列
    if ( pQueue != NULL )
    {
        for ( uint8 i = 0; i < maxNumPrepareWrites; i++ )
        {
...
/*********************************************************************
    ...
    @brief       Find client's queue.查找客户端队列
...
/*********************************************************************
    ...
    @brief   Find the Read Attribute CB function pointer for a given service.查找给定服务的读取属性回调函数指针

    @param   handle - service attribute handle 服务属性句柄
                                ...
                                uint16 value )
{
    ...
    // Write the new value for this client为此客户端写入新值
    pItem->value = value;
    return ( SUCCESS );
}
...
/*********************************************************************
    ...
    @brief   Process the client characteristic configuration
            write request for a given client.处理客户端特征配置，对给定的客户端写入请求

    @param   connHandle - connection message was received on在上接收到连接信息
    @param   pAttr - pointer to attribute
    @param   pValue - pointer to data to be written指向要写入的数据的指针
    @param   len - length of data
    @param   offset - offset of the first octet to be written要写入第一个八位字节的偏移量
    @param   validCfg - valid configuration有效配置
                                          ...
                                          uint16 validCfg )
{
    ...
    if ( offset == 0 )
    {
        if ( len == 2 )
        {
            ...
            // Validate characteristic configuration bit field验证特征配置位字段
            if ( ( value & ~validCfg ) == 0 ) // indicate and/or notify
            {
                // Write the value if it's changed如果值已经更改写入该值
...
/*********************************************************************
    ...
    @brief   Process Client Charateristic Configuration change.进程客户端特征配置更改

    @param   charCfgTbl - characteristic configuration table.特征配置表
    @param   pValue - pointer to attribute value.指向属性值的指针
    @param   authenticated - whether an authenticated link is required.是否要经过身份验证的链接
    @param   attrTbl - attribute table.属性表
    @param   numAttrs - number of attributes in attribute table.属性表中的属性数
    @param   taskId - task to be notified of confirmation.待通知确认的任务
...
//        gattCharCfg_t* pItem = &(charCfgTbl[i]);
        ...
        {
            gattAttribute_t* pAttr;
            // Find the characteristic value attribute 查找特征值属性
...
/*********************************************************************
    ...
    @brief       GATT Server Application link status change handler function.GATT服务器应用程序链接状态更改处理函数

    @param       connHandle - connection handle 连接句柄
    @param       changeType - type of change变更类型

    @return      none
**********************************************************************/
static void gattServApp_HandleConnStatusCB( uint16 connHandle, uint8 changeType )
{
    // Check to see if the connection has dropped 检查连接是否断开
              ...
              ( !linkDB_Up( connHandle ) ) ) )
    {
        ...
        // Reset Client Char Config when connection drops 连接断开时重置客户端字符配置
...
the app.生成GATT客户端配置更新事件并将其发送给应用程序
...
void GATTServApp_SendCCCUpdatedEvent( uint16 connHandle, uint16 attrHandle, uint16 value )
{
    ...
    {
        // Allocate, build and send event分配、生成、发送事件
