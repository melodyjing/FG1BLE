...
void __attribute__((used)) hal_ADC_IRQHandler(void)
{
    ...
    uint16_t adc_data[2*MAX_ADC_SAMPLE_SIZE];
		...
		switch(scan_cnt)
		{
			case 0:
				...
				if (status & BIT(5))
				{
					...
					{
						adc_data[2*n] = (uint16_t)(read_reg(ADC_CH_BASE + (ch * 0x80) + ((n+2) * 4))&0xfff);
						adc_data[2*n+1] = (uint16_t)((read_reg(ADC_CH_BASE + (ch * 0x80) + ((n+2) * 4))>>16)&0xfff);
					}
					...
					if (mAdc_Ctx.evt_handler[ch2])
					{
						...
						evt.ch = (adc_CH_t)ch2;
						evt.data = adc_data;
						evt.size = 2*(MAX_ADC_SAMPLE_SIZE-3);
						mAdc_Ctx.evt_handler[ch2](&evt);
					}

					AP_ADCC->intr_mask |= BIT(7);
					scan_cnt=1;	
...
int hal_adc_value_cal(adc_CH_t ch,uint16_t* buf, uint32_t size, uint8_t high_resol, uint8_t diff_mode)
{
//    uint32_t i;
//    int adc_sum = 0;
//    volatile float result = 0.0;
//	int result=0;
	InsertSort(buf, size);		// 排序
	uint16 sum_adc = 0;
